...
/// @notice Deploys MockUSDC to Sepolia and optionally seeds a vault address.
///
/// Usage:
...
///
/// Optional env vars:
///   SEED_ADDRESS   — address to seed with 1M mUSDC after deploy (e.g. your vault)
...
contract DeployMockUSDC is Script {
    ...
    function run() external {
        ...
        MockUSDC token = new MockUSDC(initialOwner);
        ...
        // ── Optional vault seed ────────────────────────────
        if (seedAddress != address(0)) {
            uint256 seedAmount = 1_000_000 * 10 ** 6; // 1M mUSDC
            token.ownerMint(seedAddress, seedAmount);
            console.log("Seeded", seedAmount, "mUSDC to", seedAddress);
        }
