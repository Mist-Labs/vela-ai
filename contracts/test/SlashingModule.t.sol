...
contract SlashingModuleTest is Test {
    ...
    function _buildProof(bytes32 decisionHash, bytes32 policyRoot) internal pure returns (bytes memory) {
    ...
    function test_slash_emitsEvent() public {
        ...
        emit SlashingModule.Slashed(operator, challenger, bond, challengerReward, treasuryCut, DECISION_HASH);
    ...
    function test_slash_revert_agentNotActive() public {
        ...
        vm.prank(challenger);
        vm.expectRevert(abi.encodeWithSelector(SlashingModule.AlreadySlashed.selector, operator));
        slashing.slash(operator, proof);
    }
    ...
    function test_slash_revert_agentNeverRegistered() public {
        bytes memory proof = _buildProof(DECISION_HASH, POLICY_ROOT);
        vm.prank(challenger);
        vm.expectRevert(abi.encodeWithSelector(PolicyRegistry.AgentNotRegistered.selector, stranger));
    ...
    function testFuzz_slash_split_always_sums_to_bond(uint256 bondExtra) public {
        bondExtra = bound(bondExtra, 0, 2);
        uint256[3] memory bonds = [uint256(0.05 ether), uint256(0.2 ether), uint256(1 ether)];
        uint256 bond = bonds[bondExtra];

        address richAgent = makeAddr("richAgent");
        vm.deal(richAgent, bond + 1 ether);
        vm.prank(richAgent);
        registry.registerAgent{value: bond}(POLICY_ROOT, POLICY_URI, bondExtra);
